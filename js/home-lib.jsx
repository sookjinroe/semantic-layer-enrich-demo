  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, alignItems: "start" }}>
      ...
      <U.Collapse label="Term 라이브러리 (사전)" count={L.LIBRARY.length} openByDefault accent="var(--accent)">
        <div style={{ fontSize: 14.5, color: "var(--muted)", fontFamily: "var(--sans)", marginBottom: 11, lineHeight: 1.55 }}>
          ...
          {L.LIBRARY.map((t) => (
            <div key={t.name + t.domain} style={{ border: "1px solid var(--border)", borderRadius: 4, padding: "9px 11px", background: "rgba(0,0,0,0.18)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                <span style={{ ...mono, fontSize: 15.5, color: "var(--text)", whiteSpace: "nowrap", flexShrink: 0 }}>{t.name}</span>
              ...
              <div style={{ fontSize: 14.5, color: "var(--muted)", fontFamily: "var(--sans)", lineHeight: 1.5 }}>{t.def}</div>
              {t.values && <div style={{ ...mono, fontSize: 13, color: "var(--accent)", marginTop: 4 }}>{Object.entries(t.values).map(([k, v]) => `${k}=${v}`).join("  ")}</div>}
              {t.note && <div style={{ fontSize: 14, color: "var(--low)", fontFamily: "var(--sans)", marginTop: 5, lineHeight: 1.5, opacity: 0.9 }}>↳ {t.note}</div>}
            </div>
          ))}
        </div>
      </U.Collapse>
        ...
        <U.Collapse label="BI 자산 (Looker / Tableau)" count={L.BI_ASSETS.length} openByDefault accent="var(--sig)">
          ...
          {["field", "metric", "dashboard"].map((ty) => (
            <div key={ty} style={{ marginBottom: 11 }}>
              <div style={{ ...mono, fontSize: 12.5, letterSpacing: "0.1em", color: "var(--muted)", textTransform: "uppercase", marginBottom: 6 }}>{ty} · {biByType[ty].length}</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                {biByType[ty].map((b) => {
                  const junk = /^dim_|^측정값/.test(b.label);
                  return (
                    <div key={b.id} style={{ display: "flex", gap: 9, alignItems: "baseline", fontSize: 14, fontFamily: "var(--sans)" }}>
                      <span style={{ ...mono, fontSize: 14, color: junk ? "var(--low)" : "var(--text)", minWidth: 110, flexShrink: 0, whiteSpace: "nowrap" }}>{b.label}</span>
                      <span style={{ ...mono, fontSize: 12.5, color: "var(--dim)", flexShrink: 0, minWidth: 50 }}>{b.tool}</span>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {Object.entries(lineageGroups).filter(([up]) => up !== "(상류 없음)").map(([up, cols]) => (
              <div key={up} style={{ display: "flex", gap: 9, alignItems: "baseline" }}>
                <span style={{ ...mono, fontSize: 13, color: "var(--lin)", minWidth: 168, flexShrink: 0 }}>{up}</span>
                <span style={{ ...mono, fontSize: 14, color: "var(--muted)", lineHeight: 1.5 }}>{cols.map((c) => c.asset.split(".")[1]).join(", ")}</span>
              </div>
            ))}
          </div>
        </U.Collapse>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, alignItems: "start" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        <U.Collapse label="DB 층 — schema.sql" openByDefault accent="var(--sig)">
          ...
          <pre style={{ ...mono, fontSize: 13, lineHeight: 1.55, color: "var(--text)", background: "rgba(0,0,0,0.28)", border: "1px solid var(--border)", borderRadius: 4, padding: "11px 12px", margin: 0, overflowX: "auto", whiteSpace: "pre" }}>{R.SCHEMA_SQL}</pre>
        </U.Collapse>
            ...
            {Object.entries(R.CATALOG).map(([asset, e]) => (
              <div key={asset} style={{ border: "1px solid var(--border)", borderRadius: 4, padding: "9px 11px", background: "rgba(0,0,0,0.18)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3, flexWrap: "wrap" }}>
                  <span style={{ ...mono, fontSize: 14, color: "var(--dim)" }}>{asset.split(".")[1]}</span>
                  <span style={{ ...mono, fontSize: 12 }}>→</span>
                  <span style={{ ...mono, fontSize: 15, color: "var(--accent)", whiteSpace: "nowrap", flexShrink: 0 }}>{e.term.name}</span>
                  {e.term.values
                    ? <U.Tag bg="rgba(78,201,138,0.1)" fg="var(--high)">허용값 정의됨</U.Tag>
                    : <U.Tag bg="rgba(224,107,94,0.1)" fg="var(--low)">허용값 미정</U.Tag>}
                </div>
                <div style={{ fontSize: 14.5, color: "var(--muted)", fontFamily: "var(--sans)" }}>{e.term.def}</div>
                {e.term.values && <div style={{ ...mono, fontSize: 13, color: "var(--accent)", marginTop: 4 }}>{Object.entries(e.term.values).map(([k, v]) => `${k}=${v}`).join("  ")}</div>}
              </div>
            ))}
          </div>
        </U.Collapse>
      ...
      <U.Collapse label="Code 층 — code/*.java" count={R.CODE_FILES.length + " 파일"} openByDefault accent="var(--high)">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
          {R.CODE_FILES.map((f) => (
            <div key={f.file}>
              <div style={{ ...mono, fontSize: 13, color: "var(--high)", marginBottom: 4 }}>{f.file}</div>
              <pre style={{ ...mono, fontSize: 12.5, lineHeight: 1.5, color: "var(--text)", background: "rgba(0,0,0,0.28)", border: "1px solid var(--border)", borderRadius: 4, padding: "10px 11px", margin: 0, overflowX: "auto", whiteSpace: "pre" }}>{f.body}</pre>
            </div>
          ))}
        </div>
      </U.Collapse>
