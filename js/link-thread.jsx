  ...
  return (
    <div style={{ display: "flex", gap: 13 }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0, width: 14 }}>
        {isVerdict
          ? <span style={{ color, fontSize: 15.5, lineHeight: "16px", marginTop: 2 }}>{ev.decision === "match" ? "✓" : "—"}</span>
  ...
  return (
    <div style={{ paddingTop: 1 }}>
      <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
        <span key={ev.status} className="fadeIn" style={{ ...mono, fontSize: 15, color: "var(--sig)", letterSpacing: "0.03em" }}>
      ...
      {!requesting && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginTop: 7 }}>
          {items.length === 0 && <span style={{ ...mono, fontSize: 14, color: "var(--dim)" }}>일치 후보 없음 — 매칭 실패 가능</span>}
          {items.map((cd) => (
            <span key={cd.name} style={{ ...mono, fontSize: 13, color: "var(--text)", border: "1px solid var(--border)", borderRadius: 3, padding: "2px 8px", background: "rgba(0,0,0,0.25)" }}>
...
function LTThink({ ev }) {
  if (ev.pending && !ev.text) return <span style={{ fontSize: 15.5, color: "var(--muted)", fontStyle: "italic", animation: "pulse 1.1s infinite" }}>판단 중…</span>;
  return <span style={{ fontSize: 16, lineHeight: 1.65, color: "#aab1bd" }}>{ev.text}</span>;
}
...
function LTEvidence({ U, ev }) {
  ...
  return (
    <div style={{ paddingTop: 1 }}>
      <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
        <span key={ev.status} className="fadeIn" style={{ ...mono, fontSize: 15, letterSpacing: "0.03em", color: ev.kind === "fk" ? "var(--lin)" : "var(--sig)" }}>
  ...
  if (ev.kind === "term_detail") {
    ...
    return (
      <U.SignalBlock tone="term" mono={false}>
        ...
        <div style={{ fontFamily: "var(--mono)", fontSize: 13, color: "var(--muted)", marginTop: 5 }}>도메인 {d.domain} · 허용값 {valstr} · 연결자산 {d.linked && d.linked.length ? d.linked.join(", ") : "없음"}</div>
      </U.SignalBlock>
    );
  }
  return null;
  ...
  return (
    <div style={{ paddingTop: 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        <span style={{ ...mono, fontSize: 13, color: "var(--muted)", letterSpacing: "0.06em" }}>판정</span>
        {matched
          ? <><span style={{ ...mono, fontSize: 17, color: "var(--text)" }}>{ev.term}</span><U.ConfBadge c={ev.confidence} /></>
          : <span style={{ ...mono, fontSize: 15.5, color: "var(--muted)" }}>{ev.reason === "collision" ? "이름 충돌 — 매칭 거부" : ev.reason === "scope" ? "입도 불일치 — 매칭 거부" : "부합 Term 없음"} <span style={{ color: "var(--accent)" }}>→ 발견으로 이관</span></span>}
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ padding: "13px 16px", borderBottom: "1px solid var(--border)", background: "rgba(0,0,0,0.15)" }}>
        <div style={{ ...mono, fontSize: 17, color: "var(--text)" }}>{asset}</div>
      ...
      <div style={{ padding: "16px 16px 8px" }}>
        {evs.length === 0 && <div style={{ fontSize: 15.5, color: "var(--muted)", fontStyle: "italic", animation: "pulse 1.1s infinite" }}>스레드 시작 중…</div>}
